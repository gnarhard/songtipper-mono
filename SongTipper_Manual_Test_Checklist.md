...
> Mirror of `web/app/Support/TestChecklist/ManualTestChecklist.php`. Keep both in lockstep.
...
- [ ] Payment methods pie chart shows breakdown by method (card, Apple Pay, Google Pay, cash, unknown)
- [ ] Earnings Over Time bar chart and Payment Methods pie chart appear side-by-side on wider screens (≥520 dp)
...
- [ ] Recent Performances card shows last 10 songs (title, artist, source, first-time icon, request badge, earnings)
- [ ] "View All" button on Recent Performances card opens Performance History screen
- [ ] Performance History screen groups songs by session with date/venue/gig-type header
- [ ] Performance History screen supports pagination (Load more)
...
- [ ] Edit min_suggested_setlist_songs / max_suggested_setlist_songs (1..100, min <= max)
...
- [ ] Reward threshold form: curated icon picker (12 Material icons) selects `reward_icon`
- [ ] Reward threshold form: optional description textarea (max 500 chars) sets `reward_description`
- [ ] Reward threshold form: 422 validation errors render inline per field instead of snackbar
- [ ] New project auto-creates default $40 `free_request` reward threshold (label "Free Song Request", icon `music_note`, repeating)
...
### Members & Multi-User Sync

- [ ] Project members list (Settings)
- [ ] Owner: remove member from project (with confirmation dialog)
- [ ] Share setlist with all project members (owner-only button, from Setlist Detail)
...
- [ ] Pull Owner Copy action visible on member's song in Song Detail (`source_project_song_id` set)
- [ ] Pull Owner Copy button on member's song from Setlist Detail (fetches owner's current version as alternate)
...
- [ ] Home screen: venue list tiles navigate to Venue Detail screen
- [ ] Home screen: search icon in Venues header opens Venue Search screen
- [ ] Venue Detail (personal): shows session count, total tips, tip range by day, most requested songs, most lucrative songs
- [ ] Venue Detail (personal): AI Insights card shown when available
- [ ] Venue Detail (personal): "You haven't played here yet" empty state when no qualifying sessions
- [ ] Venue Detail (crowd): "Community Insights" section shown with performer count badge when N ≥ 3
- [ ] Venue Detail (crowd): "Not enough data from other performers yet" shown when N < 3
- [ ] Venue Detail (crowd-only via search): no personal section shown; crowd section shown normally
- [ ] Venue Search: debounced search by name (≥ 2 chars triggers API call)
- [ ] Venue Search: result tiles show performer count badge when hasAnalytics = true
- [ ] Venue Search: result tiles show "N/3" indicator when hasAnalytics = false
- [ ] Venue Search: "No venues found." empty state for non-matching query
- [ ] Venue Search: tapping a result navigates to crowd-only Venue Detail
- [ ] Venue Detail (personal): Sessions section lists past sessions with date, gig type badge, and song count
- [ ] Venue Detail (personal): first tap on delete icon arms row (red highlight + Cancel button)
- [ ] Venue Detail (personal): tapping Cancel disarms the row without deleting
- [ ] Venue Detail (personal): second tap on delete icon removes session from list optimistically
- [ ] Venue Detail (personal): Your Stats section updates after session delete (session count decrements)
- [ ] Venue Detail (personal): Sessions section hidden when no sessions for venue
...
- [ ] Reward progress rows render emoji from `reward_icon` (via `RewardIconOptions::emoji`) when set
- [ ] Reward progress rows render `reward_description` as subtext below the label when present
...
- [ ] Visit /project/{slug}/suggest-setlist as anonymous user
- [ ] Page shows only public songs (is_public=true or public_repertoire_set_id members)
- [ ] Instrumental songs display with " (instrumental)" suffix
- [ ] Search filters by title or artist
- [ ] Pagination preserves picked selection across pages
- [ ] Picking a song adds it to the picked strip in insertion order
- [ ] Remove button drops a song from the picked strip
- [ ] Clear All empties the picked strip
- [ ] Submit disabled below min_suggested_setlist_songs
- [ ] Submit disabled above max_suggested_setlist_songs
- [ ] Valid submission shows thank-you panel
- [ ] Performer receives email when notify_on_request=true
- [ ] Performer receives no email when notify_on_request=false
- [ ] Honeypot fill silently discards
- [ ] Submission within 3s of load silently discards
- [ ] Rate limit: 4th submission from same IP same day is blocked
...
- [ ] Project limit — Free/Pro/Veteran all unlimited
- [ ] Repertoire song limit — Free/Pro/Veteran all unlimited
- [ ] Audience requesting (public tips + requests) — yes on Free (until `card_needed`), yes on Pro/Veteran
- [ ] Queue access — yes on all tiers
- [ ] Request history — yes on all tiers
- [ ] Owner stats — yes on all tiers
- [ ] YTD stats history chart — yes on all tiers
- [ ] Wallet view — yes on all tiers
- [ ] Invite members (band sync) — yes on all tiers
- [ ] "Top Earner" verified badge — Veteran-only
