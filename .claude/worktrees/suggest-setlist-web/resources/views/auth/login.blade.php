            ...
            <x-ui.panel class="w-full max-w-[420px] p-6 sm:p-7 shadow-lg">
                ...
                <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <x-text-input id="password" type="password" name="password" required autocomplete="current-password" class="block w-full py-2.5 pl-10 pr-3 text-sm" />
                        </div>
                        @error('password')
                            <p class="mt-2 text-sm text-rose-600 dark:text-rose-300">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <label for="remember_me" class="inline-flex items-center gap-2 text-sm text-ink-muted dark:text-ink-soft">
                            <input id="remember_me" type="checkbox" name="remember" @checked(old('remember')) class="h-4 w-4 rounded border-ink-border text-brand-600 focus:ring-brand-500 dark:border-ink-border-dark dark:bg-surface-elevated dark:text-brand-300 dark:focus:ring-brand-300">
                            <span>Remember me</span>
                        </label>

                        @if (Route::has('password.request'))
                            <x-ui.text-link :href="route('password.request')" class="text-sm">
                                Forgot password?
                            </x-ui.text-link>
                        @endif
                    </div>
                ...
                @if (Route::has('register'))
                    <p class="mt-5 text-center text-sm text-ink-muted dark:text-ink-soft">
                        Need an account?
                        <x-ui.text-link :href="route('register')">
                            Create one here
                        </x-ui.text-link>
                    </p>
                @endif
